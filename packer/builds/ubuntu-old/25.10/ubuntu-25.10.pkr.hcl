...
locals {
  ...
  // Derived Variables
  template_name        = "ubuntu-25.10-${local.version}"
  template_description = "Ubuntu 25.10\nVersion:${local.version}\nbuilt on ${local.build_date}"

  vm_name = "packer-ubuntu-25-10"
...
source "proxmox-iso" "ubuntu" {
  ...
  boot_command = [
    "c<wait5>",
    "linux /casper/vmlinuz --- autoinstall ds='nocloud;seedfrom=http://{{.HTTPIP}}:{{.HTTPPort}}'",
    "<enter><wait10>",
  ...
  boot_iso {
    iso_file     = "${var.common_iso_storage}:iso/ubuntu-25.10-live-server-amd64.iso"
    unmount      = true
    iso_checksum = "https://releases.ubuntu.com/25.10/SHA256SUMS"
  }
...
build {
  ...
  provisioner "ansible" {
    ...
    extra_arguments = [
      ...
      "--extra-vars", "ansible_key='${var.build_private_key_file}'",
